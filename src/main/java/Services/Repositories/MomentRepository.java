 ...
 */
@Repository(value = RequestContract.MOMENT_REPOSITORY_NAME)
public class MomentRepository extends BasicRepository {
    ...
    public MyModel getOne(final int id) throws Exception {
        ...
        MyModel myModel = (MyModel) session.get(Moment.class, id);
        session.close();
        return myModel;
    }
    ...
    public List<MyModel> getListAll() throws Exception {
        Session session = HibernateUtil.getInstace().getSessionFactory().openSession();
        session.beginTransaction();
        List<MyModel> listMyModel = session.createCriteria(Moment.class).list();
        session.close();
        return listMyModel;
    }
}
